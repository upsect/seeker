...
internals.error_factory = function ( opts ) {
  ...
  if ( opts.module && opts.module.length === 1 ) {
    return `Expected \`module\` to contain multiple values.
    If you only need to import a single module, use the native
    \`require\` method provided by Node.js.`;
  }
  return null;
};
...
seeker.import = function ( opts ) {
  return internals.check_opts( opts, ( error, type ) => {
    if ( error ) throw error;

    if ( type === 'module' ) {
      return internals.import_module( opts.module );
    }
